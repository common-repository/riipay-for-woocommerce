...
function riipay_custom_price_html( $price_html, $product )
{
    ...
    //get minimum product price from variations
    if ( $product->is_type('variable') ) {
        $productPrice = floatval($product->get_variation_price());
    }
    ...
    $url = add_query_arg( array(
        'merchant_code' => $merchant_code, 'amount' => $productPrice
        ), $url . '/preview');
    $html = $price_html;
    ...
    $instalmentPrice = number_format(round($productPrice / $instalmentCount, 2), 2, '.', '');
